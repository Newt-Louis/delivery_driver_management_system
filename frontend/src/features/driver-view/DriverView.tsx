...
function getDriverBrand(delivery: DeliveryRegistration): DriverUnitBrand {
...
function computeStt(delivery: DeliveryRegistration, allDeliveries: DeliveryRegistration[]): string {
  ...
  const unitDeliveries = allDeliveries.filter((d) => (d.unitConfigId ?? d.receivingUnit) === unit);
...
function QueueContext({
  ...
  const unitCalled  = allDeliveries.filter((d) => (d.unitConfigId ?? d.receivingUnit) === unit && d.status === 'CALLED');
  const unitWaiting = allDeliveries.filter((d) => (d.unitConfigId ?? d.receivingUnit) === unit && d.status === 'WAITING');
